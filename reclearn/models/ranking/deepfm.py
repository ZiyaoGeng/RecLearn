...
class DeepFM(Model):
	def __init__(self, fea_cols, hidden_units=(200, 200, 200), dnn_dropout=0.,
		...
		self.embed_dim = self.fea_cols[0]['embed_dim']  # all sparse features have the same embed_dim
		self.fm = New_FM(self.feature_length, fm_w_reg)
		self.dnn = MLP(hidden_units, activation, dnn_dropout)
