...
def create_criteo_dataset(file, embed_dim=8, read_part=True, sample_num=100000, test_size=0.2):
    ...
    # former_idx = 0
    # for i, feat_name in enumerate(dense_features + sparse_features):
    #     idx_features.append('idx_' + feat_name)
    #     if feat_name in dense_features:
    #         data_df['idx_' + feat_name] = former_idx
    #         former_idx += 1
    #     else:
    #         data_df['idx_' + feat_name] = former_idx + data_df[feat_name]
    #         former_idx += feature_columns[1][i - len(dense_features)]['feat_num']
